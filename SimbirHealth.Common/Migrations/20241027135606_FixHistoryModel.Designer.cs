// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SimbirHealth.Common;

#nullable disable

namespace SimbirHealth.Common.Migrations
{
    [DbContext(typeof(SimbirHealthContext))]
    [Migration("20241027135606_FixHistoryModel")]
    partial class FixHistoryModel
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.AccountModel", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Guid");

                    b.ToTable("Accounts");

                    b.HasData(
                        new
                        {
                            Guid = new Guid("9dd5f073-265b-4b57-8268-d0a53355b7e7"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            FirstName = "admin",
                            IsDeleted = false,
                            LastName = "default",
                            Password = "21232F297A57A5A743894A0E4A801FC3",
                            Username = "admin"
                        },
                        new
                        {
                            Guid = new Guid("dfa3ea95-21e1-44c6-9393-5ab531d39acd"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            FirstName = "manager",
                            IsDeleted = false,
                            LastName = "default",
                            Password = "1D0258C2440A8D19E716292B231E3190",
                            Username = "manager"
                        },
                        new
                        {
                            Guid = new Guid("2018473b-0ec4-4702-bbaf-667e4843a48a"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            FirstName = "doctor",
                            IsDeleted = false,
                            LastName = "default",
                            Password = "F9F16D97C90D8C6F2CAB37BB6D1F1992",
                            Username = "doctor"
                        },
                        new
                        {
                            Guid = new Guid("c6645389-3937-4d85-80e2-05437a15241b"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            FirstName = "user",
                            IsDeleted = false,
                            LastName = "default",
                            Password = "EE11CBB19052E40B07AAC0CA060C23EE",
                            Username = "user"
                        });
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.AccountToRole", b =>
                {
                    b.Property<Guid>("AccountGuid")
                        .HasColumnType("uuid");

                    b.Property<Guid>("RoleGuid")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.HasKey("AccountGuid", "RoleGuid");

                    b.HasIndex("RoleGuid");

                    b.ToTable("AccountToRole");

                    b.HasData(
                        new
                        {
                            AccountGuid = new Guid("9dd5f073-265b-4b57-8268-d0a53355b7e7"),
                            RoleGuid = new Guid("816dca08-d141-4fd1-8f34-7d7a4322a53d"),
                            IsDeleted = false
                        },
                        new
                        {
                            AccountGuid = new Guid("dfa3ea95-21e1-44c6-9393-5ab531d39acd"),
                            RoleGuid = new Guid("ac5328b1-acec-4739-b570-90bf511a3e02"),
                            IsDeleted = false
                        },
                        new
                        {
                            AccountGuid = new Guid("2018473b-0ec4-4702-bbaf-667e4843a48a"),
                            RoleGuid = new Guid("929a852e-4d8e-4595-9fee-00076e7a8a7b"),
                            IsDeleted = false
                        },
                        new
                        {
                            AccountGuid = new Guid("c6645389-3937-4d85-80e2-05437a15241b"),
                            RoleGuid = new Guid("803f5318-c437-47ce-8781-97719a4095ba"),
                            IsDeleted = false
                        });
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.RefreshToken", b =>
                {
                    b.Property<string>("Token")
                        .HasColumnType("text");

                    b.Property<Guid>("AccountGuid")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ExpiredDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.HasKey("Token");

                    b.HasIndex("AccountGuid")
                        .IsUnique();

                    b.ToTable("RefreshTokens");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.Role", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("RoleName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Guid");

                    b.ToTable("Roles");

                    b.HasData(
                        new
                        {
                            Guid = new Guid("816dca08-d141-4fd1-8f34-7d7a4322a53d"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsDeleted = false,
                            RoleName = "Admin"
                        },
                        new
                        {
                            Guid = new Guid("ac5328b1-acec-4739-b570-90bf511a3e02"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsDeleted = false,
                            RoleName = "Manager"
                        },
                        new
                        {
                            Guid = new Guid("929a852e-4d8e-4595-9fee-00076e7a8a7b"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsDeleted = false,
                            RoleName = "Doctor"
                        },
                        new
                        {
                            Guid = new Guid("803f5318-c437-47ce-8781-97719a4095ba"),
                            DateCreate = new DateTime(2024, 9, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsDeleted = false,
                            RoleName = "User"
                        });
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.History.HistoryModel", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Data")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("DoctorGuid")
                        .HasColumnType("uuid");

                    b.Property<Guid>("HospitalGuid")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<Guid>("PacientGuid")
                        .HasColumnType("uuid");

                    b.Property<Guid>("RoomGuid")
                        .HasColumnType("uuid");

                    b.HasKey("Guid");

                    b.HasIndex("DoctorGuid");

                    b.HasIndex("HospitalGuid");

                    b.HasIndex("PacientGuid");

                    b.HasIndex("RoomGuid");

                    b.ToTable("Histories");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Hospital.HospitalModel", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("ContactPhone")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Guid");

                    b.ToTable("Hospitals");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Hospital.Room", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("HospitalGuid")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Guid");

                    b.HasIndex("HospitalGuid");

                    b.ToTable("Rooms");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Timetable.Appointment", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("AccountGuid")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsTaken")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("Time")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("TimetableGuid")
                        .HasColumnType("uuid");

                    b.HasKey("Guid");

                    b.HasIndex("AccountGuid");

                    b.HasIndex("TimetableGuid");

                    b.ToTable("Appointments");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Timetable.TimetableModel", b =>
                {
                    b.Property<Guid>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("DoctorGuid")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("From")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("HospitalGuid")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<Guid>("RoomGuid")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("To")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Guid");

                    b.HasIndex("DoctorGuid");

                    b.HasIndex("HospitalGuid");

                    b.HasIndex("RoomGuid");

                    b.ToTable("Timetables");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.AccountToRole", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Account")
                        .WithMany("AccountToRoles")
                        .HasForeignKey("AccountGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Account.Role", "Role")
                        .WithMany("AccountToRoles")
                        .HasForeignKey("RoleGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.RefreshToken", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Account")
                        .WithOne("RefreshToken")
                        .HasForeignKey("SimbirHealth.Data.Models.Account.RefreshToken", "AccountGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.History.HistoryModel", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Doctor")
                        .WithMany()
                        .HasForeignKey("DoctorGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Hospital.HospitalModel", "Hospital")
                        .WithMany()
                        .HasForeignKey("HospitalGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Pacient")
                        .WithMany()
                        .HasForeignKey("PacientGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Hospital.Room", "Room")
                        .WithMany()
                        .HasForeignKey("RoomGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Doctor");

                    b.Navigation("Hospital");

                    b.Navigation("Pacient");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Hospital.Room", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Hospital.HospitalModel", "Hospital")
                        .WithMany("Rooms")
                        .HasForeignKey("HospitalGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Hospital");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Timetable.Appointment", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Account")
                        .WithMany("Appointments")
                        .HasForeignKey("AccountGuid");

                    b.HasOne("SimbirHealth.Data.Models.Timetable.TimetableModel", "Timetable")
                        .WithMany("Appointments")
                        .HasForeignKey("TimetableGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Timetable");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Timetable.TimetableModel", b =>
                {
                    b.HasOne("SimbirHealth.Data.Models.Account.AccountModel", "Doctor")
                        .WithMany("Timetables")
                        .HasForeignKey("DoctorGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Hospital.HospitalModel", "Hospital")
                        .WithMany("Timetables")
                        .HasForeignKey("HospitalGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SimbirHealth.Data.Models.Hospital.Room", "Room")
                        .WithMany("Timetables")
                        .HasForeignKey("RoomGuid")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Doctor");

                    b.Navigation("Hospital");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.AccountModel", b =>
                {
                    b.Navigation("AccountToRoles");

                    b.Navigation("Appointments");

                    b.Navigation("RefreshToken");

                    b.Navigation("Timetables");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Account.Role", b =>
                {
                    b.Navigation("AccountToRoles");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Hospital.HospitalModel", b =>
                {
                    b.Navigation("Rooms");

                    b.Navigation("Timetables");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Hospital.Room", b =>
                {
                    b.Navigation("Timetables");
                });

            modelBuilder.Entity("SimbirHealth.Data.Models.Timetable.TimetableModel", b =>
                {
                    b.Navigation("Appointments");
                });
#pragma warning restore 612, 618
        }
    }
}
